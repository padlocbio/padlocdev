...
#'   prohibited_genes = "NA"
#' )
padloc_model <- function(maximum_separation, minimum_core, minimum_total, core_genes, secondary_genes, neutral_genes, prohibited_genes) {
...
#' @title Validity checker for a padloc model
#' @description Validity checker for a padloc model.
#' @param padloc_model A padloc model.
#' @return `TRUE` if the model is valid, else an `error` object.
#' @export
valid_padloc_model <- function(padloc_model) {
...
report_padloc_model_validity <- function(models) {
  ...
  possibly_valid_padloc_model <- purrr::safely(valid_padloc_model)
  # Run across all elements
  x <- purrr::map(models, function(x) possibly_valid_padloc_model(x))
